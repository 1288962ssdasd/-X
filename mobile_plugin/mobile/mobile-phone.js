...
// 防止重复声明：如果MobilePhone已存在，跳过整个文件
if (typeof MobilePhone !== 'undefined') {
    console.log('[Mobile Phone] MobilePhone类已存在，跳过重复加载');
} else {
...
function initMobilePhone() {
    // 防止重复初始化（optimized-loader和index.js可能同时加载）
    if (window.mobilePhone) {
        console.log('[Mobile Phone] 已初始化，跳过');
        return;
    }
    // 防止class未定义（被重复声明报错后）
    if (typeof MobilePhone === 'undefined') {
        console.warn('[Mobile Phone] MobilePhone类未定义，跳过初始化');
        return;
    }
