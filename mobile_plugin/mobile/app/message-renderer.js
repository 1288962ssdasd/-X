...
if (typeof window.MessageRenderer === 'undefined') {
  ...
  window.MessageRenderer = class MessageRenderer {
    ...
    /**
     * [Fix-B] 过滤 <state> 标签，防止裸露显示在界面上
     * AI 可能输出 <state>变量名=值</state> 格式的状态栏，
     * 在手机消息气泡中应隐藏或替换为简洁文字
     */
    removeStateTags(text) {
      if (!text) return '';
      // 移除完整的 <state>...</state> 标签及其内容
      return text.replace(/<state>[\s\S]*?<\/state>/gi, '');
    }
    ...
    renderSingleMessage(message) {
      ...
      const content = message.content || '';
      // [Fix-B] 过滤 <state> 标签，防止裸露显示在消息气泡中
      var cleanContent = this.removeStateTags ? this.removeStateTags(content) : content;
