    ...
    async loadConfigFromDataBank() {
      try {
        ...
        if (configAttachment) {
          ...
          // [修复v2] TXT格式fallback：ST的uploadFileAttachmentToServer可能把JSON保存为.txt
          // 不再直接拒绝，而是尝试下载并验证内容是否为合法JSON
          if (configAttachment.url.endsWith('.txt')) {
            console.warn('[Style Config Manager] ⚠️ 配置文件URL以.txt结尾，尝试作为JSON加载...');
          }
    ...
    async saveConfigToDataBank() {
      try {
        ...
        if (fileUrl && isValidJsonUrl) {
          ...
          // [修复v2] ST可能把JSON保存为.txt，但不一定是错误——文件内容可能仍然是合法JSON
          console.warn('[Style Config Manager] ⚠️ 文件被保存为TXT格式:', fileUrl, '（ST的uploadFileAttachmentToServer行为，内容可能仍为JSON）');
          return true;  // 不再返回false，让后续加载时尝试解析
        }
