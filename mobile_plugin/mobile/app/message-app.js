...
if (typeof window.MessageApp === 'undefined') {
  class MessageApp {
    ...
    init() {
      ...
      // [修复v3] 初始化 friendsData，避免 _deliverMessageToApp 写入时 undefined
      this.friendsData = {};

      // 立即绑定事件（包括返回按钮）
      this.bindEvents();

      // [修复v3] 监听 BridgeAPI EventBus 事件，实现变量驱动 UI 刷新
      this._setupBridgeEventListeners();
    ...
    // [修复v3] 监听 BridgeAPI EventBus 事件，实现变量驱动 UI 刷新
    // 当小白X写入 pendingMsg/pendingFriend 等变量后，BridgeAPI 处理完毕
    // 通过 EventBus 广播事件，messageApp 监听后自动刷新 UI
    _setupBridgeEventListeners() {
      ...
      function trySetup() {
        ...
        var EventBus = window.BridgeAPI.EventBus;

        // 监听好友添加事件
        EventBus.on('phone:friend_added', function(data) {
          console.log('[Message App] 收到好友添加事件:', data && data.name);
          setTimeout(function() {
            if (self.updateAppContent) {
              try { self.updateAppContent(); } catch(e) {}
            }
          }, 200);
        });

        // 监听消息通知事件
        EventBus.on('phone:notification', function(data) {
          console.log('[Message App] 收到通知事件:', data && data.title);
          setTimeout(function() {
            if (self.updateAppContent) {
              try { self.updateAppContent(); } catch(e) {}
            }
        ...
        // 监听消息发送事件
        EventBus.on('phone:message_sent', function(data) {
          console.log('[Message App] 收到消息发送事件');
          setTimeout(function() {
            if (self.updateAppContent) {
              try { self.updateAppContent(); } catch(e) {}
            }
        ...
        console.log('[Message App] BridgeAPI EventBus 监听已注册');
      }
    ...
    integrateRealTimeSync() {
      try {
        // [修复v2] real-time-sync 已退役，直接进入 fallback，不再重试报错
        console.log('[Message App] 实时同步器已退役，使用备用同步机制');
        this.setupFallbackSync();
      } catch (error) {
        console.error('[Message App] 备用同步设置失败:', error);
      }
    }
    ...
    loadRealTimeSyncModule() {
      try {
        console.log('[Message App] 🔄 实时同步器已退役，跳过加载');
      } catch (error) {
          ...
          if (sendInput && this.currentSelectedFriend) {
            ...
            if (message) {
              ...
              if (success) {
                ...
                // Skip delayed render in independent AI mode to prevent clearing bubbles
    ...
    // Append independent AI chat history to message detail view
    _appendIndependentAIHistory() {
      try {
        if (!window.independentAI || !window.independentAI.chatHistories) return;
        if (!this.currentFriendId) return;

        const history = window.independentAI.chatHistories[this.currentFriendId];
        if (!history || history.length === 0) return;

        const container = document.querySelector('.messages-container');
        if (!container) return;

        // Check if independent messages already rendered (avoid duplicates)
        // 检查容器中是否已有消息（说明已经渲染过了）
        const existingCount = container.querySelectorAll('.message-detail').length;
        if (existingCount > 0) return;

        const friendName = this.currentFriendName || '';

        // Render each history message using renderSingleMessage
        let html = '';
        history.forEach(msg => {
          const isUser = msg.role === 'user';
          const msgType = msg.messageType || '文字';
          const msgContent = msg.msgContent || msg.content;
          const fullMatch = msg.fullMatch || null;

          let fm;
          if (fullMatch) {
            fm = fullMatch;
          } else if (isUser) {
            fm = '[我方消息|我|' + this.currentFriendId + '|' + msgType + '|' + msgContent + ']';
          } else {
            fm = '[对方消息|' + friendName + '|' + this.currentFriendId + '|' + msgType + '|' + msgContent + ']';
          }

          const msgObj = {
            fullMatch: fm,
            messageType: msgType,
            sender: isUser ? '我' : friendName,
            content: msgContent,
            number: this.currentFriendId,
            isUser: isUser
          };
          const rendered = window.messageRenderer.renderSingleMessage(msgObj);
          if (rendered) {
            // Add a marker class to identify independent messages
            html += rendered.replace('message-detail ', 'message-detail independent-history-msg ');
          }
        });
        ...
        if (html) {
          ...
          // Scroll to bottom
          setTimeout(() => {
            const scrollEl = document.querySelector('.message-detail-content');
            if (scrollEl) scrollEl.scrollTop = scrollEl.scrollHeight;
          }, 100);
          console.log('[Message App] Appended', history.length, 'independent AI messages');
        }
      } catch (e) {
        console.warn('[Message App] Failed to append independent AI history:', e);
      }
    }
    ...
    async loadMessageDetailAsync() {
      try {
        ...
        if (appContent && this.currentView === 'messageDetail') {
          ...
          // Append independent AI chat history if available
          this._appendIndependentAIHistory();

          // Bind TTS click events on voice bubbles
          if (window.phoneTTS) {
            window.phoneTTS.bindVoiceBubbleEvents();
          }

          // [MR-Fix-1] 渲染完成后主动通知 quick-reply-bridge 处理新消息
          // 避免依赖 MutationObserver 被动触发，减少竞态风险
          if (window.QuickReplyBridge && typeof window.QuickReplyBridge._processExistingMessages === 'function') {
            setTimeout(function () {
              window.QuickReplyBridge._processExistingMessages();
            }, 200);
          }
        }
      } catch (error) {
...
正常内容 [好友id|赵六|666777]
  ...
  window.getMessageAppContent = function () {
    ...
    if (!window.messageApp) {
      ...
      // 刷新缓存的待处理消息（导演系统在 messageApp 创建前可能已投递消息）
      if (window.BridgeAPI && typeof window.BridgeAPI._flushPendingMessages === 'function') {
        setTimeout(function () {
          window.BridgeAPI._flushPendingMessages();
        }, 500);
      }
    }
