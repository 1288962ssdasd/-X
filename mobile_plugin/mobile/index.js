...
// 优化：使用并行加载替代顺序加载（optimized-loader 已退役，改为直接加载）
async function initOptimizedLoading() {
  try {
    // 定义核心模块（高优先级）— 仅包含底部未单独加载的模块
    const coreModules = [
      './scripts/extensions/third-party/mobile/context-monitor.js',
      './scripts/extensions/third-party/mobile/mobile-upload.js',
      './scripts/extensions/third-party/mobile/mobile-phone.js',
    ];

    // 扩展模块已由底部独立加载，此处不再重复加载

    // 并行加载核心模块
    console.log('[Mobile Context] 开始并行加载核心模块...');
    await Promise.all(coreModules.map(function(src) {
...
// independent-ai.css 已退役（样式已迁移到 role-api.js）

// [ML-Fix-2] 已删除 mobile-phone.js 的第二加载路径（原第318-345行）
// mobile-phone.js 已在 initOptimizedLoading() 中作为核心模块加载（第50行）
// 双重加载路径在竞态条件下会导致 addUploadButtonToMobilePhone 被多次调用，产生重复按钮
// 如果 initOptimizedLoading 失败，fallbackToTraditionalLoading 也会加载 mobile-phone.js
...
// 加载微信语音增强器（独立脚本，不修改原有模块）
const wechatVoiceEnhancerScript = document.createElement('script');
wechatVoiceEnhancerScript.src = './scripts/extensions/third-party/mobile/app/wechat-voice-enhancer.js';
wechatVoiceEnhancerScript.onload = () => {
  console.log('[Mobile Context] 微信语音增强器加载完成');
};
wechatVoiceEnhancerScript.onerror = () => {
  console.error('[Mobile Context] 微信语音增强器加载失败');
};
document.head.appendChild(wechatVoiceEnhancerScript);
...
async function initMobileContextPlugin() {
  // [ML-Fix-1] 防重入保护：防止插件被多次初始化
  if (isInitialized) {
    console.log('[Mobile Context] 插件已初始化，跳过重复调用');
    return;
  }
 ...
 */
// [ML-Fix-3] 递归重试计数器
var _initRetryCount = { initMesIDFloorMonitor: 0, initForumFeatures: 0, initWeiboFeatures: 0 };
var MAX_INIT_RETRIES = 10;

function initMesIDFloorMonitor() {
  try {
    if (!window.mesidFloorMonitor) {
      // [ML-Fix-3] 添加最大重试限制，防止无限递归
      if (_initRetryCount.initMesIDFloorMonitor >= MAX_INIT_RETRIES) {
        console.error('[Mobile Context] MesID楼层监听器初始化超时，已重试', MAX_INIT_RETRIES, '次');
        return;
      }
      _initRetryCount.initMesIDFloorMonitor++;
      console.warn('[Mobile Context] MesID楼层监听器未就绪，等待中... (', _initRetryCount.initMesIDFloorMonitor, '/', MAX_INIT_RETRIES, ')');
      // 等待楼层监听器加载完成
...
function initForumFeatures() {
  try {
    ...
    if (!window.forumManager) {
      // [ML-Fix-3] 添加最大重试限制
      if (_initRetryCount.initForumFeatures >= MAX_INIT_RETRIES) {
        console.error('[Mobile Context] 论坛管理器初始化超时，已重试', MAX_INIT_RETRIES, '次');
        return;
      }
      _initRetryCount.initForumFeatures++;
      console.warn('[Mobile Context] 论坛管理器未就绪，等待中... (', _initRetryCount.initForumFeatures, '/', MAX_INIT_RETRIES, ')');
      // 等待论坛管理器加载完成
...
function initWeiboFeatures() {
  try {
    ...
    if (!window.weiboManager) {
      // [ML-Fix-3] 添加最大重试限制
      if (_initRetryCount.initWeiboFeatures >= MAX_INIT_RETRIES) {
        console.error('[Mobile Context] 微博管理器初始化超时，已重试', MAX_INIT_RETRIES, '次');
        return;
      }
      _initRetryCount.initWeiboFeatures++;
      console.warn('[Mobile Context] 微博管理器未就绪，等待中... (', _initRetryCount.initWeiboFeatures, '/', MAX_INIT_RETRIES, ')');
      // 等待微博管理器加载完成
